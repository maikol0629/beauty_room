...
class TelegramUpdateHandlerTest {
    ...
    void menuText_shouldListServicesWithInlineKeyboard() {
        ...
        Service haircut = Service.builder().id(1L).name_service("Haircut").price(15.99f).duration(30).build();
    ...
    void selectService_shouldAskForDateWithAvailableDays() {
        ...
        Service haircut = Service.builder().id(1L).name_service("Haircut").stylist(stylist).build();
    ...
    void selectTime_shouldAskForConfirmation() {
        ...
        Stylist stylist = Stylist.builder().id(5L).name_stylist("John Doe").build();
        Service haircut = Service.builder().id(1L).name_service("Haircut").build();
        when(serviceRepository.findByIdAndTenantId(1L, 1L)).thenReturn(Optional.of(haircut));
    ...
    void confirm_newChat_shouldCreateClientAndSaveAppointment() {
        ...
        verify(clientRepository).save(argThat(c ->
                c.getTenant().getId().equals(1L)
                        && "111".equals(c.getTelegram_chat_id())
                        && "Juan".equals(c.getName_client())));
        verify(channel).sendMessage(eq("111"), contains("¡Listo!"));
    }
